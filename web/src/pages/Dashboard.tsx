...
export default function DashboardPage() {
  ...
  const loading = projectsLoading || historyLoading;
  ...
  return (
    <div className="page-shell space-y-6 md:space-y-8">
      ...
      {!loading && projects.length === 0 && (
        <div className="card p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-4 justify-between">
          <div>
            <div className="panel-title">先建立一个工作空间</div>
            <p className="page-subtitle mt-1">
              项目是接口、环境变量和编排流的边界。创建项目后，顶部栏会记住当前工作上下文。
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {(loading
          ? [
              { label: "项目", value: null, icon: Boxes, color: "from-brand to-accent" },
              { label: "接口", value: null, icon: Activity, color: "from-emerald-500 to-teal-400" },
              { label: "环境", value: null, icon: BarChart3, color: "from-sky-500 to-cyan-400" },
              { label: "编排", value: null, icon: Workflow, color: "from-amber-400 to-orange-500" },
            ]
          : [
          { label: "项目", value: projects.length, icon: Boxes, color: "from-brand to-accent" },
          { label: "接口", value: totalEndpoints, icon: Activity, color: "from-emerald-500 to-teal-400" },
          { label: "环境", value: totalEnvironments, icon: BarChart3, color: "from-sky-500 to-cyan-400" },
          ...
          >
            ...
            {s.value === null ? (
              <div className="skeleton-line w-16 h-8 mt-2" />
            ) : (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div className="panel-title">近 7 天运行</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="text-center rounded-lg bg-bg-elevated/45 py-3">
              <div className="text-lg font-semibold text-white tabular-nums">{runStats.total}</div>
              <div className="text-[11px] text-slate-500">总次数</div>
            </div>
            <div className="text-center rounded-lg bg-bg-elevated/45 py-3">
              <div className="text-lg font-semibold text-emerald-500 dark:text-emerald-400 tabular-nums">{runStats.success}</div>
              <div className="text-[11px] text-slate-500">成功</div>
            </div>
            <div className="text-center rounded-lg bg-bg-elevated/45 py-3">
              <div className="text-lg font-semibold text-rose-500 dark:text-rose-400 tabular-nums">{runStats.failed}</div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-[11px]">
              ...
              <span className={`font-medium tabular-nums ${healthTone}`}>{hasRuns ? `${successRate}%` : "暂无数据"}</span>
            </div>
          ...
          <div className="mt-3 pt-3 border-t border-white/5">
            <div className="flex items-center justify-between text-[11px] text-slate-500">
              ...
              <span className="font-mono text-slate-300 tabular-nums">{hasRuns ? `${runStats.avgDuration}ms` : "—"}</span>
            </div>
          </div>
            </>
          )}
            ...
            <div className="space-y-1">
              {lastFive.map((h) => (
                <Link
                  key={h.id}
                  to="/history"
                  className="soft-row flex items-center gap-2 px-3 py-2 text-xs"
                >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="panel-title">项目概况</div>
            </div>
          ...
          {projects.length === 0 ? (
            ...
            <div className="space-y-1">
              {projects.map((p) => (
                <div
                  key={p.id}
                  className="soft-row flex items-center justify-between px-3 py-2 group"
                >
