...
function EnvVarPopover({
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full right-0 mt-2 w-80 z-50 rounded-xl border border-white/10 bg-bg-panel shadow-2xl overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
        <div>
          <div className="text-xs font-medium text-white">{env.name}</div>
      ...
      <div className="px-4 py-2 border-t border-white/5 text-[10px] text-slate-600">
...
export default function TopBar() {
  ...
  return (
    <div className="h-14 border-b border-black/[0.06] dark:border-white/5 bg-white/60 dark:bg-bg-panel/40 backdrop-blur-xl flex items-center px-3 md:px-5 gap-2 md:gap-3 shrink-0">
      {/* Project selector */}
      <div className="flex items-center gap-1.5 min-w-0">
        ...
        <Select
          ...
          placeholder="选择项目…"
          className="text-xs md:text-sm w-[130px] md:w-[180px]"
        />
      </div>
      ...
      <div className="flex items-center gap-1 min-w-0">
        ...
        <Select
          ...
          className="text-xs md:text-sm w-[110px] md:w-[160px]"
        />
      ...
      <button
        className="hidden lg:flex items-center gap-2 text-xs text-slate-500 hover:text-slate-300 transition-colors border border-white/8 rounded-lg px-3 py-1.5 hover:border-white/15"
        title="全局搜索接口 (⌘K)"
        onClick={() => {
